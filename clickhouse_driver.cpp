...
#include <iostream>
// The structure defined for a packet is used to encapsule the data for transfer
// between dpdk main application and clickhouse driver
#include "packet.h"
...
void CH::insert() {
  ...
  */
  ...
  // std::cout << "Number of rows in block: " << block.GetRowCount() <<
  // std::endl; block.AppendColumn("name", name);
...
void CH::show() {
  ...
  client->Select("SELECT pkt_length, ip_total_length FROM test.packets",
                 [](const Block &block) {
...
extern "C" void CHconstruct() { ch = new CH(); }

extern "C" void CHdestruct() { delete ch; }

extern "C" void CHappend(struct packet *p) { ch->append(p); }
extern "C" void CHinsert() { ch->insert(); }
extern "C" void CHshow() { ch->show(); }
