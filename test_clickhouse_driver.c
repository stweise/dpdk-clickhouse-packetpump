...
/*
 * Notes: clickhouse-cpp is not thread safe yet, therefore I will not try to use
 * a threads here
 */
...
void insertFakeData() {
  ...
  for (times = 0; times < 5; times++) {
    ...
    clock_gettime(
        CLOCK_MONOTONIC,
        &start);  // using a monotonic clock prevents surprises introduced by
                  // clocks changing during measurements (such as NTP)
#endif
    int i;
    struct packet p;
