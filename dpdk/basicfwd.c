...
static __attribute__((noreturn)) void lcore_main(void)
{
	...
	for (;;)
	{
		...
		if (unlikely(nb_rx == 0))
			continue;
		// filter packets, transferring from pkts_burst to whitelist_arr, those who fit the dynamic creteria
		// right now just copy everything
		//rte_memcpy(whitelist_arr, pkts_burst, sizeof(struct rte_mbuf*) * nb_rx);
		...
		for (p = 0; p < nb_rx; p++)
		{
			printf("--------------------------------------\n");
			printf("\tEther\n");
			uint32_t pkt_len = rte_pktmbuf_pkt_len(pkts_burst[p]) - sizeof(struct rte_ether_hdr);
